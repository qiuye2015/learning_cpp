...
void show_help() {
	char *help = (char*)"written by Min (http://54min.com)\n\n"
		"-d           run as a deamon\n"
...
int main(int argc, char *argv[]) {
	//自定义信号处理函数
	signal(SIGHUP, signal_handler);//关闭shell 加nohup解决
	signal(SIGTERM, signal_handler);//kill默认发出的信号
	signal(SIGINT, signal_handler);//ctrl+c 加&解决
	signal(SIGQUIT, signal_handler);// ctrl+"\"
	//ctrl + z SIGSTOP 没有捕获

	//默认参数
	char *httpd_option_listen = (char*)"0.0.0.0";
	...
	//判断是否设置了-d，以daemon运行//TODO daemon
